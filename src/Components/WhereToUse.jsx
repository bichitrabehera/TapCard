...
const WhereToUse = () => (
    <section className="w-full text-white px-6 py-30 bg-[#111111]">
        <div className="max-w-6xl mx-auto">
            {/* Header */}
            <div className="text-center mb-20">
                <p className="uppercase text-lg text-pink-600 tracking-wider mb-2">
                    TapCard In Action
                </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                {useCases.map((item, idx) => (
                    <div
                        key={idx}
                        className="group w-full max-w-sm mx-auto bg-[#000000] border border-white/10 rounded-xl p-5 sm:p-6 backdrop-blur-md hover:border-white/20 hover:shadow-cyan-500/10 transition-all duration-300"
                    >
                        <div
                            className="w-12 h-12 mb-4 rounded-full bg-white/10 flex items-center justify-center text-xl">
                            {item.icon}
                        </div>
